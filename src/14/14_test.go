...
5 BHXH, 4 VRPVC => 5 LTCX`, 2210736}}
	for _, test := range tests {
		tst.Equals(t, test.ore, parseReqs(test.in).oreCost(1))
	}
}
...
func TestTrillionOreCost(t *testing.T) {
...
5 BHXH, 4 VRPVC => 5 LTCX`, 460664},
		...
		tst.Equals(t, test.cost, parseReqs(test.in).trillionOre())
	}
}
